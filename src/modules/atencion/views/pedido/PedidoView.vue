            ...
            <v-dialog id="dlg-pedido" v-model="dialog" transition="dialog-bottom-transition" class="items-pedidos">
                <v-card color="#679a50">
                    ...
                    <v-list class="bg-dialog">
                            ...
                            <template v-slot:default>
                                <div class="d-flex justify-content-center align-items-center">
                                        ...
                                        <div class="text-right texto-cantidad">
                                            <v-tooltip text="Disminuir unidades" location="bottom">
                                                <template v-slot:activator="{ props }">
                                                    <v-btn v-if="estado <= 1" icon="mdi-minus-thick" size="25px" color="#856826" v-bind="props" @click="item.amount <= 1 ? item.amount = 1 : item.amount--; actualizarItem(item.id, item.amount)"></v-btn>
                                                </template>
                                            </v-tooltip>
                                            ...
                                            <v-tooltip text="Aumentar unidades" location="bottom">
                                                <template v-slot:activator="{ props }">
                                                    <v-btn v-if="estado <= 1" class="me-1" icon="mdi-plus-thick" size="25px" color="#856826" v-bind="props" @click="item.amount >= 1000 ? item.amount = 1000 : item.amount++; actualizarItem(item.id, item.amount)"></v-btn>
                                                </template>
                                            </v-tooltip>
                                            <v-tooltip text="Eliminar item de la orden" location="bottom">
                                                <template v-slot:activator="{ props }">
                                                    <v-btn v-if="estado <= 1" icon="mdi-trash-can" size="25px" color="#E75D48" v-bind="props" @click="eliminarItem(item.id)"></v-btn>
                                                </template>
                                            </v-tooltip>
                                        </div>
                                    </div>
                                    <div v-else class="d-flex justify-content-right align-items-right">
                                        ...
                                        <div>
                                            <v-icon v-if="item.state == 0" icon="mdi-pencil" color="#343a40"></v-icon>
                        ...
                        <div v-if="toEdit" class="text-center">
                            <div v-if="addItem">
...
export default {
    ...
    methods: {
        ...
        async agregarItem(item) {
            ...
            if (this.estado <= 1) {
                const existe = this.order_details.some(i => i.menu_detail.id == item.id)
                if (existe) {
                    Swal.fire({
                        position: 'top-end',
                        text: 'El ítem ya se encuentra en el detalle del pedido',
                        color: 'white',
                        background: 'red',
                        showConfirmButton: false,
                        timer: 1200
